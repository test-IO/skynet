...
module Skynet
  class Logger
    ...
    def info(event, session_id: nil, info: {})
      log(event, session_id: session_id, info: {level: :info}.merge(info))
    end

    def error(event, session_id: nil, info: {})
      log(event, session_id: session_id, info: {level: :error}.merge(info))
    end

    def debug(event, session_id: nil, info: {})
      log(event, session_id: session_id, info: {level: :debug}.merge(info))
    end
